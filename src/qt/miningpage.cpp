...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as Kickasscoin-Qt.", ERROR, NULL);
    }
}
...
void MiningPage::enablePoolMiningControls(bool enable)
{
    ...
    ui->serverLine->setText("127.0.0.1");
    ui->portLine->setEnabled(enable);
    ui->portLine->setText("55880");
    ui->usernameLine->setEnabled(enable);
    ui->usernameLine->setText("mindping");
    ui->passwordLine->setEnabled(enable);
    ui->passwordLine->setText("changemsoon");
}
